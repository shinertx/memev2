...
pub struct QuoteResult {
    pub out_amount: u64,
    pub price_per_token: f64, // Price of the token per USD
}
...
impl JupiterClient {
    ...
    pub async fn get_quote(&self, amount_sol_to_swap: f64, output_mint: &str) -> Result<QuoteResult> {
        ...
        let url = format!(
            ...
            self.api_url, output_mint, amount_lamports, 30 // Use fixed slippage for quote
        );
        ...
        let price_per_token = (amount_sol_to_swap / (out_amount as f64 / 1_000_000_000.0)).recip();
