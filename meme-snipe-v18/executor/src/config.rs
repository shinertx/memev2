...
impl Config {
    fn load() -> Self {
        Self {
            ...
            global_max_position_usd: env::var("GLOBAL_MAX_POSITION_USD").expect("GLOBAL_MAX_POSITION_USD must be set").parse().expect("GLOBAL_MAX_POSITION_USD must be a valid number"),
            portfolio_stop_loss_percent: env::var("PORTFOLIO_STOP_LOSS_PERCENT").expect("PORTFOLIO_STOP_LOSS_PERCENT must be set").parse().expect("PORTFOLIO_STOP_LOSS_PERCENT must be a valid number"),
            trailing_stop_loss_percent: env::var("TRAILING_STOP_LOSS_PERCENT").expect("TRAILING_STOP_LOSS_PERCENT must be set").parse().expect("TRAILING_STOP_LOSS_PERCENT must be a valid number"),
            jupiter_api_url: env::var("JUPITER_API_URL").expect("JUPITER_API_URL must be set"),
            slippage_bps: env::var("SLIPPAGE_BPS").expect("SLIPPAGE_BPS must be set").parse().expect("SLIPPAGE_BPS must be a valid number"),
            jito_tip_lamports: env::var("JITO_TIP_LAMPORTS").expect("JITO_TIP_LAMPORTS must be set").parse().expect("JITO_TIP_LAMPORTS must be a valid number"),
