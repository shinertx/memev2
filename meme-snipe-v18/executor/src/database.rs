...
impl Database {
    ...
    pub fn log_trade_attempt(&self, details: &OrderDetails, strategy_id: &str, entry_price_usd: f64) -> Result<i64> {
        ...
        self.conn.execute(
            ...
            params![
                ...
                entry_price_usd,
                details.confidence,
                details.side.to_string(), // NEW
                entry_price_usd, // NEW: highest_price_usd starts at entry_price
            ],
        )?;
    ...
    pub fn get_all_trades(&self) -> Result<Vec<TradeRecord>> {
        ...
        let trades_iter = stmt.query_map([], |row| {
            Ok(TradeRecord {
                ...
                side: row.get(13)?, // NEW
                highest_price_usd: row.get(14)?, // NEW
            })
        })?;
    ...
    pub fn get_open_trades(&self) -> Result<Vec<TradeRecord>> { // NEW: For position_manager
    ...
    pub fn update_highest_price(&self, trade_id: i64, new_highest_price: f64) -> Result<()> { // NEW: For position_manager
        self.conn.execute(
