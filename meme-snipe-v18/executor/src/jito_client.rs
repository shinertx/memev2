...
impl JitoClient {
    pub async fn new(jito_rpc_url: &str) -> Result<Self> {
        ...
        let auth_keypair = Arc::new(read_keypair_file(&auth_keypair_path)
        ...
        Ok(Self { inner, auth_keypair, rpc_client })
    }
    ...
    pub async fn send_transaction(&self, tx: &VersionedTransaction) -> Result<Signature> {
        ...
        info!("Sending bundle to Jito. First transaction signature: {}", tx.signatures);
        let _ = self.inner.send_bundle(&bundle).await.context("Failed to send Jito bundle")?;
